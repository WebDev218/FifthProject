...
/*
 use this formatting for all photolist entries. Strings only
 [filename, alt-text, caption]
 file name for thumbnail and full size should be identical
*/
...
for (let i = 0; i < photoList.length; i++ ) {
	gallery += 
	`<a href="photos/${photoList[i][0]}" class="image-tile" data-caption="${photoList[i][2]}">
	    <img src="photos/thumbnails/${photoList[i][0]}" alt="${photoList[i][1]}">
	</a>`;	
}

photoGallery.innerHTML = gallery;
baguetteBox.run('.gallery'); // place after photoGallery has been updated